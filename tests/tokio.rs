...
mod tests {
    ...
    fn repeated() {
        let signals = Signals::new(&[libc::SIGUSR1])
            ...
            .for_each(|sig| {
                ...
                Ok(send_sig(libc::SIGUSR1))
            });
        send_sig(libc::SIGUSR1);
        tokio::run(signals);
    }
    ...
    fn delayed() {
        ...
        let signals = Signals::new(&[libc::SIGUSR1, libc::SIGUSR2])
            .unwrap()
            .into_async()
            .unwrap()
            .filter(|sig| *sig == libc::SIGUSR2)
            .take(CNT as u64)
            .map_err(|e| panic!("{}", e))
            .for_each(move |_| {
        ...
        let senders = Interval::new(Instant::now(), Duration::from_millis(250))
            .map_err(|e| panic!("{}", e))
            .for_each(|_| Ok(send_sig(libc::SIGUSR2)));
